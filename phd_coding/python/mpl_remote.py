...
class Visualization:
    ...
    def plot_1d_solutions(self, data_axis, data_peak, plot_type="intensity"):
        ...
        Arguments:
            data_axis: Array containing the axis data to plot.
        ...
        ax2.plot(
            ...
            linestyle="-",
            label="On-axis maximum solution",
        )
        ax2.set(
        ...
        filename = os.path.join(self.config.output_dir, f"1d_{plot_type}.png")
            ...
            if coord_sys == "rt" and k_array is not None:
                ...
                for idx in range(len(k_array)):
                    ...
                    filename = os.path.join(
                        ...
                        f"2d_{plot_type}_{coord_sys}_{z_pos:.2f}.png",
                    )
                    plt.savefig(filename, dpi=300, bbox_inches="tight")
                    plt.close(fig)
            else:
                ...
                filename = os.path.join(
                    self.config.output_dir, f"2d_{plot_type}_{coord_sys}.png"
                )
                plt.savefig(filename, dpi=300, bbox_inches="tight")
                plt.close(fig)
        ...
        for coord_sys, data1 in data.items():
            if coord_sys == "rt" and k_array is not None:
                ...
                for idx in range(len(k_array)):
                    ...
                    filename = os.path.join(
                        self.config.output_dir,
                        f"3d_{plot_type}_{coord_sys}_{z_pos:.2f}.png",
                    )
                    plt.savefig(filename, dpi=300, bbox_inches="tight")
                    plt.close(fig)
            else:
                ...
                filename = os.path.join(
                    self.config.output_dir, f"3d_{plot_type}_{coord_sys}.png"
                )
                plt.savefig(filename, dpi=300, bbox_inches="tight")
                plt.close(fig)
...
def main():
    ...
    # Load data
    data_path = "/home/ismael/documentos_ln/doctorado/codigos/python/guia_practica/x_waves_rapidillo.npz"
    custom_output_dir = "/home/ismael/documentos_ln/doctorado/codigos/python/guia_practica/figures_rapidillo"
    ...
    # Initialize classes
    constants = UniversalConstants()
    config = VisualizationConfig(output_dir=custom_output_dir)
