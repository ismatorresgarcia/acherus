...
Numerical discretization: Finite Differences Method (FDM).
    - Method: Fourier Crank-Nicolson (FCN) scheme.
        *- Fast Fourier Transform (FFT) scheme for transforming NEE to frequency domain.
        *- Extended Crank-Nicolson (CN-RK4) scheme for solving NEE in frequency domain.
...
def create_crank_nicolson_matrix(n_r, m_p, r_low, r_up, coef_m_s, coef_o_s):
    """
...
def frequency_domain(nlin):
    """
    Transform to frequency-domain each
    nonlinear term at current propagation
    step.

    Parameters:
    - nlin: pre-allocated array for the nonlinear terms
    """
    return fft(nlin, axis=1, workers=-1)


def time_domain(fenv_c):
    """
    Transform back to time-domain the Fourier
    envelope at current propagation step.

    Parameters:
    - fenv_c: fourier envelope at current propagation step
    """
    return ifft(fenv_c, axis=1, workers=-1)
...
def _set_envelope_operator(
    ...
    Returns:
    - complex 2D-array: Combined nonlinear terms
    """
...
def _rk4_envelope_step(env, dens, ram, steep_op, env_rk4, env_args, dz, dz_2, dz_6):
    """Combined RK4 step for all nonlinear terms."""
    k1_env = _set_envelope_operator(env, dens, ram, steep_op, *env_args)
    env_rk4 = env + dz_2 * time_domain(k1_env)

    k2_env = _set_envelope_operator(env_rk4, dens, ram, steep_op, *env_args)
    env_rk4 = env + dz_2 * time_domain(k2_env)

    k3_env = _set_envelope_operator(env_rk4, dens, ram, steep_op, *env_args)
    env_rk4 = env + dz * time_domain(k3_env)

    k4_env = _set_envelope_operator(env_rk4, dens, ram, steep_op, *env_args)
...
class NEEParameters:
    ...
    def __init__(self, const, medium, domain, laser):
        ...
        self._init_operators(const, medium, domain, laser)
...
class FCNSolver:
    """Fourier Crank-Nicolson solver class for beam propagation."""
