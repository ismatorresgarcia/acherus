...
def crank_nicolson_step(lm, rm, e_c, e_n):
    """
...
for k in tqdm(range(N_STEPS)):
    crank_nicolson_step(
        left_operator, right_operator, current_envelope[k, :], next_envelope
    )
...
plot_int_sol = AREA_FACTOR * MEDIA["WATER"]["INT_FACTOR"] * np.abs(envelope_solol) ** 2
