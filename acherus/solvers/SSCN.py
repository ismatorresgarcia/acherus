...
class SSCN(Shared):
    ...
    def compute_dispersion(self, w_det, w_0):
        ...
        if self.dispersion_model == "full":
            w = w_det + w_0
            _, k_w, _ = self.medium.dispersion_properties(w)
            return k_w - self.k_0 - self.k_1 * w_det

        if self.dispersion_model == "partial":
            d_w = 0.5 * self.dispersion_par.k2 * w_det**2
            if self.dispersion_par.k3 is not None:
                d_w += (1.0 / 6.0) * self.dispersion_par.k3 * w_det**3
            if self.dispersion_par.k4 is not None:
                d_w += (1.0 / 24.0) * self.dispersion_par.k4 * w_det**4
            if self.dispersion_par.k5 is not None:
                d_w += (1.0 / 120.0) * self.dispersion_par.k5 * w_det**5
            return d_w

        raise ValueError(f"Invalid dispersion model: '{self.dispersion_model}'.")
