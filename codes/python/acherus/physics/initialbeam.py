...
def initialize_envelope(grid, laser):
    ...
    """
    r_grid2d, t_grid2d = np.meshgrid(grid.r_grid, grid.t_grid, indexing="ij")
    exp_r2 = -((r_grid2d / laser.waist) ** laser.gauss_opt).astype(np.complex128)
    exp_t2 = -((t_grid2d / laser.duration) ** 2).astype(np.complex128)

    if laser.focal_length != 0:  # phase curvature due to the focusing lens
        exp_r2 -= 0.5j * laser.wavenumber * r_grid2d**2 / laser.focal_length
    if laser.chirp != 0:  # temporal phase due to the chirping system
        exp_t2 -= 1j * laser.chirp * (t_grid2d / laser.duration) ** 2
